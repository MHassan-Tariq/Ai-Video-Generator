...
export default function ExamplePhotosPage() {
  ...
  const [deletingItem, setDeletingItem] = useState<ExamplePhoto | null>(null)
  ...
  const handleOpenDialog = (item?: ExamplePhoto) => {
    if (item) {
      setEditingItem(item)
      setFormData({
        title: item.title || "",
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await updateDocument("example_photos", editingItem.id, formData)
      } else {
        await addDocument("example_photos", formData)
      }
      await loadData()
  ...
  const handleDeleteClick = (item: ExamplePhoto) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setIsDeleting(true)
      await deleteDocument("example_photos", deletingItem.id)
  ...
  const columns: ColumnDef<ExamplePhoto>[] = [
    {
      accessorKey: "image",
      header: "Photo",
      cell: ({ row }) => (
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-card p-6 rounded-xl border border-border shadow-sm">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Example Photos</h2>
          <p className="text-muted-foreground">Manage gallery images for home decoration inspiration.</p>
        </div>
      ...
      <div className="bg-card p-4 rounded-xl border border-border shadow-sm">
         {loading ? (
             <div className="text-center p-8 text-muted-foreground">Loading photos...</div>
         ) : (
